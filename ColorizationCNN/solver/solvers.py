from .layers import conv_forward_naive, conv_back_naive, relu, relu_back, max_pooling, fully_connected, \
            ...
            for b in range(epoch_size):
                ...
                if i % print_every == 0:
                    ...
                    accuracy_summary = tf.Summary(
                        value=[tf.Summary.Value(tag='Validation Accuracy', simple_value=validation_accuracy)])
                    writer.add_summary(accuracy_summary, i)

                    data = [["Progress", "Mini Batch Cost", "Validation Accuracy"],
    ...
    def eval(self, inputs, batches=None):
        ...
        if batches is not None:
            ...
            for b in range(epoch):
                ...
                cost, caches = self.forward_propagate(batch_input, self._weights, self._params, self._bn_params,
                                                      run='test')
